...
Set-Alias -Name 'browse' -Value 'explorer.exe'
Set-Alias -Name 'ibrowse' -Value 'MicrosoftEdge.exe'

function wup {
    'winget upgrade --all' | Invoke-Expression
}
...
function clh {
    ...
    Get-ChildItem -Path $env:USERPROFILE\Downloads | Remove-Item -Force -Recurse -ErrorAction SilentlyContinue
    Remove-Item -Path $env:APPDATA\Microsoft\Windows\PowerShell\PSReadLine\ -Recurse -Force -ErrorAction SilentlyContinue
    Remove-Item -Path $env:USERPROFILE\.bash_history -Force -ErrorAction Ignore
    Get-ChildItem -Path $env:TEMP | Remove-Item -Force -Recurse -ErrorAction SilentlyContinue
    Clear-RecycleBin -DriveLetter 'C:\', 'D:\' -Force -ErrorAction SilentlyContinue
    Clear-DnsClientCache
    if ($e) {
        'dism /online /cleanup-image /startcomponentcleanup /resetbase /defer' | Invoke-Expression
    }
}
